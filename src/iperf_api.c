...
iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
    ...
    while ((flag = getopt_long(argc, argv, "p:zU:y:f:i:DVJvsc:ub:t:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:h", longopts, NULL)) != -1) {
        switch (flag) {
            case 'p':
            ...
            case 'i':
                ...
                setinterval(test->stats_interval);
                break;
            case 'D':
		test->daemon = 1;
		server_flag = 1;
		...
		setcurrentrole(test->role);

		iperf_set_test_server_hostname(test, optarg);
		setcurrentip(test->server_hostname);
                break;
            case 'u':
